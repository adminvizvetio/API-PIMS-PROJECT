...
import { defaultEzyAccounts } from "@/utils/utils";

export default function EzyListManager() {
  const [accounts, setAccounts] = useState<any[]>(defaultEzyAccounts);
  const [inputValue, setInputValue] = useState<string>("");
  const [status, setStatus] = useState(false);

  const [isOpenEzyVetModal, setIsOpenEzyVetModal] = useState(false);
  const [selectedClient, setSelectedClient] = useState<any>();

  const [errorMessage, setErrorMessage] = useState("");

  useEffect(() => {
    const storedAccounts = localStorage.getItem(STORAGE_EZY_ACCOUNTS);
    if (storedAccounts) {
  ...
  useEffect(() => {
    if (status) {
      localStorage.setItem(STORAGE_EZY_ACCOUNTS, JSON.stringify(accounts));
    }
  }, [accounts]);
  ...
  const openEzyVetModal = () => setIsOpenEzyVetModal(true);
  const closeEzyVetModal = () => setIsOpenEzyVetModal(false);
    ...
    <div className="w-full mx-auto">
      ...
      <div className="flex m-10">
        <input
          ...
          placeholder="Add New EzyVet Client"
        />
        <div className="flex w-1/6 justify-end">
