...
export async function POST(request: Request) {
  ...
  const tableName = `${prefix}contacts`;
  ...
  try {
    ...
    const values = content
      .map((item) => {
        const { id, code, ownership_id, created_at, modified_at } =
          item.contact;
        return `('${id}', '${escapeSQL(code)}', '${ownership_id}', '${created_at}', '${modified_at}', '${escapeSQL(accountId)}', '${new Date().toISOString().slice(0, 19).replace("T", " ")}')`;
      })
      .join(",");

    let query = `INSERT INTO ${tableName} (ezyvet_id, code, ownership_id, created_at, updated_at, account_id, inserted_at) VALUES ${values}`;
